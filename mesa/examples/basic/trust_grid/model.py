...
from agents import TrustAgent


class TrustGrid(mesa.Model):
    ...
    def __init__(
        ...
        TrustAgent.create_agents(
            self, len(self.grid.all_cells.cells), cell=self.grid.all_cells.cells
        )

        self.datacollector = mesa.DataCollector(
            {
                "Investors": lambda m: len(
                    [a for a in m.agents if a.move == "I"]
                ),
                "Trustworthy_Trustees": lambda m: len(
                    [a for a in m.agents if a.move == "T"]
                ),
                "Untrustworthy_Trustees": lambda m: len(
                    [a for a in m.agents if a.move == "U"]
                ),
            }
        )
